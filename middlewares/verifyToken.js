  ...
  try {
    ...
    try {
      ...
      if (refreshToken === undefined) {
        ...
        return res.status(244).json({ message: 'Access forbidden, invalid refreshToken' });
      }
      ...
      if (refreshToken && refreshToken === storedToken.refreshToken) {
        const newAccessToken = jwt.sign({ userId: storedToken.userId }, `${process.env.JWT_SECRET_KEY}`, {
          expiresIn: '3h',
        });
        const newRefreshToken = jwt.sign({ userId: storedToken.userId }, `${process.env.JWT_SECRET_KEY}`, {
          expiresIn: '1d',
        });
